...
| Table | Field Name | Data Type | Coverage | PIT Possible? | Usage Notes |
|-------|------------|-----------|----------|---------------|-------------|
| `ria_contacts_current` | `REP_LICENSES` | **STRING** | 100% | ❌ **NO** | ⚠️ **STRING not ARRAY** - JSON format, current state only. **Note**: Only contains regulatory Series licenses, NOT professional certifications |
...
- ⚠️ **Important**: `REP_LICENSES` only contains regulatory Series licenses. For professional certifications (CFP, CFA, etc.), see Professional Certifications section below

### Professional Certifications

| Table | Field Name | Data Type | Coverage | PIT Possible? | Usage Notes |
|-------|------------|-----------|----------|---------------|-------------|
| `ria_contacts_current` | `CONTACT_BIO` | **STRING** | ~60-70% | ❌ **NO** | **Primary source** - Unstructured biographical text containing certification mentions |
| `ria_contacts_current` | `TITLE_NAME` | STRING | ~85% | ❌ **NO** | **Secondary source** - Job title field that may include certifications |

⚠️ **Critical**: Professional certifications (CFP, CFA, ChFC, CLU, CAIA, etc.) are **NOT** stored in `REP_LICENSES`. `REP_LICENSES` only contains regulatory Series licenses.

**Certification Coverage** (as of January 2025):
- **CFA** (Chartered Financial Analyst): ~10,352 advisors in `CONTACT_BIO`
- **CFP** (Certified Financial Planner): ~8,668 advisors in `CONTACT_BIO`
- **ChFC** (Chartered Financial Consultant): ~2,845 advisors in `CONTACT_BIO`, ~14 in `TITLE_NAME`
- **CLU** (Chartered Life Underwriter): ~2,302 advisors in `CONTACT_BIO`
- **CAIA** (Chartered Alternative Investment Analyst): ~601 advisors in `CONTACT_BIO`

**Query Syntax**:
```sql
-- Extract certifications from CONTACT_BIO (most comprehensive source)
SELECT 
    RIA_CONTACT_CRD_ID,
    CASE WHEN CONTACT_BIO LIKE '%CFP%' OR CONTACT_BIO LIKE '%Certified Financial Planner%' THEN 1 ELSE 0 END as has_cfp,
    CASE WHEN CONTACT_BIO LIKE '%CFA%' OR CONTACT_BIO LIKE '%Chartered Financial Analyst%' OR CONTACT_BIO LIKE '%CFA charterholder%' THEN 1 ELSE 0 END as has_cfa,
    CASE WHEN CONTACT_BIO LIKE '%ChFC%' OR CONTACT_BIO LIKE '%Chartered Financial Consultant%' THEN 1 ELSE 0 END as has_chfc,
    CASE WHEN CONTACT_BIO LIKE '%CLU%' OR CONTACT_BIO LIKE '%Chartered Life Underwriter%' THEN 1 ELSE 0 END as has_clu,
    CASE WHEN CONTACT_BIO LIKE '%CAIA%' OR CONTACT_BIO LIKE '%Chartered Alternative Investment Analyst%' THEN 1 ELSE 0 END as has_caia
FROM `savvy-gtm-analytics.FinTrx_data_CA.ria_contacts_current`
WHERE CONTACT_BIO IS NOT NULL;

-- Also check TITLE_NAME for additional coverage (sparse but structured)
SELECT 
    RIA_CONTACT_CRD_ID,
    TITLE_NAME,
    CASE WHEN TITLE_NAME LIKE '%CFP%' THEN 1 ELSE 0 END as has_cfp,
    CASE WHEN TITLE_NAME LIKE '%CFA%' THEN 1 ELSE 0 END as has_cfa,
    CASE WHEN TITLE_NAME LIKE '%ChFC%' THEN 1 ELSE 0 END as has_chfc,
    CASE WHEN TITLE_NAME LIKE '%CLU%' THEN 1 ELSE 0 END as has_clu
FROM `savvy-gtm-analytics.FinTrx_data_CA.ria_contacts_current`
WHERE TITLE_NAME IS NOT NULL
  AND (TITLE_NAME LIKE '%CFP%'
       OR TITLE_NAME LIKE '%CFA%'
       OR TITLE_NAME LIKE '%ChFC%'
       OR TITLE_NAME LIKE '%CLU%');
```

**Feature Engineering Recommendations**:
- Create binary flags: `has_cfp`, `has_cfa`, `has_chfc`, `has_clu`, `has_caia` (extract from `CONTACT_BIO`)
- Count total number of professional certifications held
- Create composite feature: `has_advanced_certification` (CFA or CFP)
- ⚠️ **Data Leakage Warning**: Both `CONTACT_BIO` and `TITLE_NAME` are current state only (not point-in-time). Using these for historical predictions creates a small data leakage risk, as certifications may have been earned after the `contacted_date`.
...
| `ria_contacts_current` | `TITLE_NAME` | STRING | ~85% | ❌ **NO** | Job title, current state only. May contain professional certifications (CFP, CFA, ChFC, CLU, etc.) - see Professional Certifications section |
