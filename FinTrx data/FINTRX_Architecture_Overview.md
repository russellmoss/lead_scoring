...
|-----------|---------------|---------|----------------|
| **Rep's Licenses** | ❌ **NO** | Only current state stored | `ria_contacts_current.REP_LICENSES` (Series licenses only) |
| **Rep's Certifications** | ❌ **NO** | Only current state stored | `ria_contacts_current.CONTACT_BIO`, `ria_contacts_current.TITLE_NAME` |
...
- ❌ No historical tracking of contact AUM, licenses, certifications, or other contact-level metrics
...
- `REP_LICENSES` - Series licenses held (STRING, not ARRAY - use JSON_EXTRACT_ARRAY()). **Note**: Only contains regulatory Series licenses, NOT professional certifications (CFP, CFA, etc.)
- `CONTACT_BIO` - Biographical text containing professional certifications (CFP, CFA, ChFC, CLU, CAIA, etc.) - **primary source for certifications**
- `TITLE_NAME` - Job title that may include professional certifications - **secondary source for certifications**
...
| **PIT Capabilities** | Firm data: ✅ | Employment: ✅ | Accolades: ✅ | Licenses: ❌ | Certifications: ❌ |
