...
| `REP_LICENSES` | **STRING** | 0% | **JSON array string** of **regulatory licenses only** (Series exams) | ⚠️ **NOT ARRAY** - use `JSON_EXTRACT_ARRAY()` or `LIKE '%Series 7%'`. **Note**: Does NOT contain professional certifications (CFP, CFA, etc.) - see `CONTACT_BIO` and `TITLE_NAME` |
| `REP_TYPE` | STRING | ~5% | Rep type | Values: "DR", "BD", "IA", "NA" |
| `PRODUCING_ADVISOR` | BOOLEAN | ~10% | Whether rep is a producing advisor | |
| `TITLE_NAME` | STRING | ~85% | Job title and professional designations | May contain certifications (CFP®, CFA, ChFC®, CLU®, etc.) - see Professional Certifications section |
| `CONTACT_BIO` | STRING | ~60-70% | Unstructured biographical text | **Primary source for professional certifications** (CFP, CFA, ChFC, CLU, CAIA, etc.) - see Professional Certifications section |
...
⚠️ **Important**: Professional certifications (CFP, CFA, ChFC, CLU, CAIA, etc.) are **NOT** stored in `REP_LICENSES`. `REP_LICENSES` only contains regulatory Series licenses (Series 7, Series 65, etc.).

**Certifications are found in:**
1. **`CONTACT_BIO`** (primary source) - Unstructured biographical text containing certification mentions
2. **`TITLE_NAME`** (secondary source) - Job title field that may include certifications
...
**Query Examples**:
```sql
-- Extract certifications from CONTACT_BIO (most comprehensive)
SELECT 
    RIA_CONTACT_CRD_ID,
    CASE WHEN CONTACT_BIO LIKE '%CFP%' OR CONTACT_BIO LIKE '%Certified Financial Planner%' THEN 1 ELSE 0 END as has_cfp,
    CASE WHEN CONTACT_BIO LIKE '%CFA%' OR CONTACT_BIO LIKE '%Chartered Financial Analyst%' THEN 1 ELSE 0 END as has_cfa,
...
-- Also check TITLE_NAME for additional coverage
SELECT 
    RIA_CONTACT_CRD_ID,
    TITLE_NAME
FROM `savvy-gtm-analytics.FinTrx_data_CA.ria_contacts_current`
