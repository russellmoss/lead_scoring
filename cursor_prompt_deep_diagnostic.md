# Cursor Prompt: Deep Diagnostic - Target Variable & Feature Analysis

## Context

CV fix applied but performance still at 1.50x (target: 2.62x). The key mystery:
- `flight_risk_score` shows +657% univariate signal
- But it's rank #19 in model importance

We need to investigate:
1. Target variable definition match
2. `flight_risk_score` distribution (sparsity issue?)
3. Time period performance (2024 vs 2025)
...
df_v3 = client.query(query_v3_target).result().to_dataframe()
...
df_status = client.query(query_mql_check).result().to_dataframe()
...
try:
    df_compare = client.query(query_v2_compare).result().to_dataframe()
...
frs_lift = top_decile_frs['target'].mean() / y_test.mean()
...
WHERE table_name = 'lead_scoring_splits'
"""

v3_columns = client.query(query_v3_columns).result().to_dataframe()['column_name'].tolist()
...
|------------|-------------------|
| **#1: Target Variable** | Is v3's target defined the same as v2? |
| **#2: Sparsity** | What % of leads have flight_risk_score > 0? |
| **#3: Time Period** | Does signal strength differ 2024 vs 2025? |
| **#4: Single Feature** | What lift can flight_risk_score achieve alone? |
| **#5: V2 Features** | Are we missing any v2 features (like mobility tiers)? |

## Expected Findings

If we see:
- **flight_risk_score = 0 for 80%+ of leads** → Sparsity is the problem, need to bin or transform
- **Single feature lift > 2x** → Model architecture issue (not learning the signal)
- **Missing v2 features** → Add them back (especially mobility tiers, aum_growth)
- **Target rates differ between v2/v3** → Target definition is wrong
