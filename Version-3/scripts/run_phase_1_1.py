"""
Phase 1.1: Point-in-Time Feature Engineering Architecture
Generates and executes the complete PIT feature engineering SQL using Virtual Snapshot methodology
"""
...
def generate_feature_engineering_sql(analysis_date: str, maturity_window_days: int, 
                                     training_start_date: str, training_end_date: str) -> str:
    """
    Generate complete PIT feature engineering SQL.
    This is a large SQL query - we'll build it in sections.
    """
...
-- ANALYSIS DATE: {analysis_date} (fixed for training set stability)
-- MATURITY WINDOW: {maturity_window_days} days
-- ============================================================================
...
lead_base AS (
    ...
    WHERE l.stage_entered_contacting__c IS NOT NULL
        ...
        AND DATE(l.stage_entered_contacting__c) >= '{training_start_date}'
        AND DATE(l.stage_entered_contacting__c) <= '{training_end_date}'
),
...
advisor_features_virtual AS (
    SELECT
        ...
        (SELECT 
            AVG(DATE_DIFF(
                COALESCE(eh_prior.PREVIOUS_REGISTRATION_COMPANY_END_DATE, DATE('{analysis_date}')),
...
def run_phase_1_1():
    """Execute Phase 1.1: Point-in-Time Feature Engineering"""
    ...
    maturity_window_days = date_config['maturity_window_days']
    training_start_date = date_config['training_start_date']
    training_end_date = date_config['training_end_date']
    
    logger.log_metric("Analysis Date", analysis_date)
    logger.log_metric("Maturity Window", f"{maturity_window_days} days")
    logger.log_metric("Training Start", training_start_date)
    logger.log_metric("Training End", training_end_date)
    
    # Generate SQL
    logger.log_action("Generating PIT feature engineering SQL")
    sql = generate_feature_engineering_sql(
    ...
    # Save SQL to file
    sql_path = BASE_DIR / "sql" / "lead_scoring_features_pit.sql"
    with open(sql_path, 'w', encoding='utf-8') as f:
        f.write(sql)
    logger.log_file_created("lead_scoring_features_pit.sql", str(sql_path),
                           "Complete PIT feature engineering SQL")
    
    # Execute SQL
    logger.log_action("Executing feature engineering SQL (this may take several minutes)")
    ...
    try:
        ...
        logger.log_learning("Feature engineering SQL executed successfully")
        
        # Get row count
        count_query = "SELECT COUNT(*) as row_count FROM `savvy-gtm-analytics.ml_features.lead_scoring_features_pit`"
        count_result = client.query(count_query, location="northamerica-northeast2").to_dataframe()
        row_count = count_result['row_count'].iloc[0]
        
        logger.log_metric("Total Rows Created", f"{row_count:,}")
        
        # Get positive/negative counts
        target_query = """
        SELECT 
            COUNT(*) as total,
            COUNTIF(target = 1) as positives,
            COUNTIF(target = 0) as negatives,
            ROUND(COUNTIF(target = 1) * 100.0 / COUNT(*), 2) as positive_rate
        FROM `savvy-gtm-analytics.ml_features.lead_scoring_features_pit`
        """
        target_result = client.query(target_query, location="northamerica-northeast2").to_dataframe()
        target_stats = target_result.to_dict('records')[0]
        
        logger.log_metric("Positive Leads", target_stats['positives'])
        logger.log_metric("Negative Leads", target_stats['negatives'])
        logger.log_metric("Positive Rate", f"{target_stats['positive_rate']}%")
        
        # Validation gates
        if row_count >= 3000:
            logger.log_validation_gate("G1.1.1", "Feature Table Row Count", True,
                                      f"{row_count:,} rows (>=3,000 required)")
        else:
            logger.log_validation_gate("G1.1.1", "Feature Table Row Count", False,
                                      f"Only {row_count:,} rows (need >=3,000)")
        
        if 2 <= target_stats['positive_rate'] <= 6:
            logger.log_validation_gate("G1.1.2", "Positive Class Rate", True,
                                      f"{target_stats['positive_rate']}% (within 2-6% range)")
        else:
            logger.log_validation_gate("G1.1.2", "Positive Class Rate", False,
                                      f"{target_stats['positive_rate']}% (outside 2-6% range)")
        
        # Check for data leakage (PIT audit)
        logger.log_action("Running PIT leakage audit")
        leakage_query = """
        SELECT 
        ...
        """
        leakage_result = client.query(leakage_query, location="northamerica-northeast2").to_dataframe()
        leakage_stats = leakage_result.to_dict('records')[0]
        
        if leakage_stats['leakage_count'] == 0:
            logger.log_validation_gate("G1.1.3", "PIT Integrity (No Leakage)", True,
                                      "Zero leakage detected - all features use data available at or before contacted_date")
        else:
            logger.log_validation_gate("G1.1.3", "PIT Integrity (No Leakage)", False,
                                      f"{leakage_stats['leakage_count']} rows with pit_month > contacted_date (LEAKAGE DETECTED)")
        
        logger.end_phase(
            status="PASSED",
            next_steps=["Proceed to Phase 2: Training Dataset Construction"]
        )
        
    except Exception as e:
        logger.log_validation_gate("G1.1.0", "SQL Execution", False, str(e))
...
if __name__ == "__main__":
    run_phase_1_1()
    print("\n=== PHASE 1.1 COMPLETE ===")
    print("Feature engineering table created: ml_features.lead_scoring_features_pit")
