-- Week 3 Validation Checkpoint 3.3: Build "Hybrid (Stable/Mutable)" Training Set & Validate

-- Uses discovery_reps_current (has all features + snapshot_as_of)

-- Optionally includes discovery_firms_current for firm-level features
...
AllLeadsWithSnapshot AS (
    SELECT 
        ...
        -- Day_of_Contact will be derived in FinalDatasetCreation from Stage_Entered_Contacting__c
        ...
        -- Is_Weekend_Contact will be derived in FinalDatasetCreation from Day_of_Contact
        drc.KnownNonAdvisor,
        drc.LastName,
        drc.Licenses,
        drc.MilesToWork,
        ...
        -- dfc.total_firm_aum_millions as Firm_TotalAUM,

        -- dfc.avg_rep_aum_millions as Firm_AvgRepAUM,

        -- ... (add firm features as needed)
...
FinalDatasetCreation AS (
    SELECT 
        ...
        -- Temporal features (derived from Stage_Entered_Contacting__c):
        -- BigQuery EXTRACT(DAYOFWEEK) returns 1=Sunday, 7=Saturday
        -- We want 1=Monday, 7=Sunday to match plan spec
        CASE 
            WHEN EXTRACT(DAYOFWEEK FROM Stage_Entered_Contacting__c) = 1 THEN 7  -- Sunday -> 7
            ELSE EXTRACT(DAYOFWEEK FROM Stage_Entered_Contacting__c) - 1  -- Monday(2)->1, ..., Saturday(7)->6
        END as Day_of_Contact,  -- 1=Monday, 2=Tuesday, ..., 7=Sunday
        CASE 
            WHEN EXTRACT(DAYOFWEEK FROM Stage_Entered_Contacting__c) IN (1, 7) THEN 1  -- Saturday or Sunday
            ELSE 0 
        END as Is_Weekend_Contact,  -- 1=Saturday/Sunday, 0=Weekday
