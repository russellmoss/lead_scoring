...
-- Overall and 30-day conversion and imbalance (all contacts)
WITH L AS (
  SELECT 
    Id,
    Stage_Entered_Contacting__c AS contacted_ts,
    Stage_Entered_Call_Scheduled__c AS call_ts
  FROM `savvy-gtm-analytics.SavvyGTMData.Lead`
  WHERE Stage_Entered_Contacting__c IS NOT NULL
)
SELECT 
  COUNT(*) AS total_contacts,
  COUNT(CASE WHEN call_ts IS NOT NULL THEN 1 END) AS all_time_conversions,
  ROUND(COUNT(CASE WHEN call_ts IS NOT NULL THEN 1 END) / COUNT(*) * 100, 2) AS all_time_conv_rate_pct,
  COUNT(CASE WHEN call_ts IS NOT NULL AND DATE(call_ts) <= DATE_ADD(DATE(contacted_ts), INTERVAL 30 DAY) THEN 1 END) AS conv_30d,
  ROUND(COUNT(CASE WHEN call_ts IS NOT NULL AND DATE(call_ts) <= DATE_ADD(DATE(contacted_ts), INTERVAL 30 DAY) THEN 1 END) / COUNT(*) * 100, 2) AS conv_30d_rate_pct,
  SAFE_DIVIDE(COUNT(CASE WHEN call_ts IS NULL OR DATE(call_ts) > DATE_ADD(DATE(contacted_ts), INTERVAL 30 DAY) THEN 1 END), 
              NULLIF(COUNT(CASE WHEN call_ts IS NOT NULL AND DATE(call_ts) <= DATE_ADD(DATE(contacted_ts), INTERVAL 30 DAY) THEN 1 END),0)) AS class_imbalance_ratio_30d
FROM L;

-- Discovery coverage (CRD match rate) among contacted leads
WITH S AS (
  SELECT Id, FA_CRD__c
  FROM `savvy-gtm-analytics.SavvyGTMData.Lead`
  WHERE Stage_Entered_Contacting__c IS NOT NULL
...
SELECT 
  ...
  COUNT(CASE WHEN FA_CRD__c IS NOT NULL THEN 1 END) AS leads_with_crd,
  COUNT(dd.RepCRD) AS leads_matched_discovery,
  ROUND(COUNT(dd.RepCRD) / NULLIF(COUNT(CASE WHEN FA_CRD__c IS NOT NULL THEN 1 END),0) * 100, 2) AS crd_match_rate_pct
FROM S
LEFT JOIN `savvy-gtm-analytics.LeadScoring.discovery_reps_current` dd
  ON S.FA_CRD__c = dd.RepCRD;

-- AUM tier performance (from Lead.Total_Firm_AUM__c), 30-day window
WITH L AS (
  SELECT 
    Id,
    Total_Firm_AUM__c AS firm_aum,
    Stage_Entered_Contacting__c AS contacted_ts,
    Stage_Entered_Call_Scheduled__c AS call_ts
  FROM `savvy-gtm-analytics.SavvyGTMData.Lead`
  WHERE Stage_Entered_Contacting__c IS NOT NULL
  ...
  SELECT 
    Id,
    CASE 
      WHEN firm_aum IS NULL THEN 'Unknown'
      WHEN firm_aum < 100000000 THEN '<$100M'
      WHEN firm_aum >= 100000000 AND firm_aum < 500000000 THEN '$100-500M'
      ELSE '>$500M'
    END AS aum_tier,
    contacted_ts,
    call_ts
...
GROUP BY aum_tier
ORDER BY 
  CASE aum_tier WHEN '<$100M' THEN 1 WHEN '$100-500M' THEN 2 WHEN '>$500M' THEN 3 ELSE 4 END;

-- Metro area performance based on discovery home metro flags
WITH L AS (
  SELECT 
    Id,
    FA_CRD__c,
  ...
  SELECT 
    ...
    L.call_ts,
    CASE 
      WHEN dd.Home_MetropolitanArea_New_York_Newark_Jersey_City_NY_NJ = 1 THEN 'NYC'
      WHEN dd.Home_MetropolitanArea_Los_Angeles_Long_Beach_Anaheim_CA = 1 THEN 'LA'
      WHEN dd.Home_MetropolitanArea_Chicago_Naperville_Elgin_IL_IN = 1 THEN 'Chicago'
      WHEN dd.Home_MetropolitanArea_Dallas_Fort_Worth_Arlington_TX = 1 THEN 'Dallas'
      WHEN dd.Home_MetropolitanArea_Miami_Fort_Lauderdale_West_Palm_Beach_FL = 1 THEN 'Miami'
      ELSE 'Other'
    END AS metro
  FROM L
  LEFT JOIN `savvy-gtm-analytics.LeadScoring.discovery_reps_current` dd
    ON L.FA_CRD__c = dd.RepCRD
)
SELECT 
  metro,
  COUNT(*) AS leads,
...
FROM J
GROUP BY metro
ORDER BY CASE metro WHEN 'NYC' THEN 1 WHEN 'LA' THEN 2 WHEN 'Chicago' THEN 3 WHEN 'Dallas' THEN 4 WHEN 'Miami' THEN 5 ELSE 6 END;

-- Temporal patterns: day-of-week and weekend performance (30-day window)
WITH L AS (
  SELECT 
    Id,
    Stage_Entered_Contacting__c AS contacted_ts,
...
SELECT 
  EXTRACT(DAYOFWEEK FROM contacted_ts) AS dow, -- 1=Sunday, 7=Saturday
  CASE WHEN EXTRACT(DAYOFWEEK FROM contacted_ts) IN (1,7) THEN 1 ELSE 0 END AS is_weekend,
  COUNT(*) AS leads,
  COUNT(CASE WHEN call_ts IS NOT NULL AND DATE(call_ts) <= DATE_ADD(DATE(contacted_ts), INTERVAL 30 DAY) THEN 1 END) AS conv_30d,
  ROUND(COUNT(CASE WHEN call_ts IS NOT NULL AND DATE(call_ts) <= DATE_ADD(DATE(contacted_ts), INTERVAL 30 DAY) THEN 1 END)/COUNT(*)*100,2) AS conv_30d_rate_pct
FROM L
GROUP BY dow, is_weekend
ORDER BY dow;
-- ===================================================================
-- STEP 3.1: CLASS DISTRIBUTION ANALYSIS
