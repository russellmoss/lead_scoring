"""
Unit 4 V4: Model Training (PII Removed) (with m5 Feature Engineering)
Reads from BigQuery, applies m5 feature engineering, tests imbalance strategies, trains final models.
V4: Removes PII and simple geographic features to force the model to use real business signals.
"""
import json
import os
import math
...
BQ_TABLE = "savvy-gtm-analytics.LeadScoring.step_3_3_training_dataset_v2"  # V4 uses same dataset as V2

# PII and simple geographic features to remove (V4)
PII_TO_DROP = [
    'FirstName', 'LastName', 
...
def main():
    ...
    print("=" * 80)
    print("Unit 4 V4: Model Training (PII Removed)")
    print("=" * 80)
    print()
    ...
    for fold_id, (train_idx, test_idx) in enumerate(folds):
        ...
        print(f"      V4 (PII removed): using {len(kept_features)}/{len(feature_cols)} features", flush=True)
    ...
    # V4: No pre-filtering - use all features (PII already removed)
    print(f"[9/13] Preparing full dataset (V4: Using all features, PII removed)...", flush=True)
    ...
    joblib.dump(final_model, ARTIFACTS_DIR / "model_v4.pkl")
    joblib.dump(baseline_model, ARTIFACTS_DIR / "model_v4_baseline_logit.pkl")
    
    # Save CV fold indices
    with open(ARTIFACTS_DIR / "cv_fold_indices_v4.json", "w", encoding="utf-8") as f:
        json.dump(cv_indices, f, indent=2)
    
    # Save selected features
    with open(ARTIFACTS_DIR / "selected_features_v4.json", "w", encoding="utf-8") as f:
    ...
    importance_df.to_csv(ARTIFACTS_DIR / "feature_importance_v4.csv", index=False)
    ...
    with open(ARTIFACTS_DIR / "feature_selection_report_v4.md", "w", encoding="utf-8") as f:
    ...
    with open(ARTIFACTS_DIR / "model_training_report_v4.md", "w", encoding="utf-8") as f:
    ...
    print(f"  - model_v4.pkl")
    print(f"  - model_v4_baseline_logit.pkl")
    print(f"  - cv_fold_indices_v4.json")
    print(f"  - feature_importance_v4.csv")
    print(f"  - feature_selection_report_v4.md")
    print(f"  - model_training_report_v4.md")
    print(f"  - selected_features_v4.json")
    print()
