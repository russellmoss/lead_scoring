"""
Unit 4 V5: Model Training (PII Removed + Strong Regularization) (with m5 Feature Engineering)
Reads from BigQuery, applies m5 feature engineering, tests imbalance strategies, trains final models.
V5: Removes PII and simple geographic features, adds strong regularization to prevent overfitting.
"""
import json
import os
import math
...
BQ_TABLE = "savvy-gtm-analytics.LeadScoring.step_3_3_training_dataset_v2"  # V5 uses same dataset as V2

# PII and simple geographic features to remove (V5 - same as V4)
PII_TO_DROP = [
    'FirstName', 'LastName', 
...
def main():
    ...
    print("=" * 80)
    print("Unit 4 V5: Model Training (PII Removed + Strong Regularization)")
    print("=" * 80)
    print()
    ...
    # V4: Remove PII and simple geographic features before feature engineering
    print(f"[2.5/13] Removing PII and simple geographic features...", flush=True)
    pii_cols_to_drop = [col for col in PII_TO_DROP if col in df.columns]
    if pii_cols_to_drop:
        df = df.drop(columns=pii_cols_to_drop)
        print(f"   Dropped {len(pii_cols_to_drop)} PII/simple geographic features: {', '.join(pii_cols_to_drop)}", flush=True)
    else:
    ...
    for fold_id, (train_idx, test_idx) in enumerate(folds):
        ...
        print(f"      V5 (PII removed, regularized): using {len(kept_features)}/{len(feature_cols)} features", flush=True)
    ...
    # V5: No pre-filtering - use all features (PII already removed)
    print(f"[9/13] Preparing full dataset (V5: Using all features, PII removed, with strong regularization)...", flush=True)
    ...
    # CRITICAL: Save the exact feature order as an official artifact
    # This ensures all future scripts (calibration, backtesting) use the same order
    final_feature_order = X_all.columns.tolist()
    feature_order_path = ARTIFACTS_DIR / "feature_order_v5.json"
    with open(feature_order_path, 'w', encoding='utf-8') as f:
        json.dump(final_feature_order, f, indent=2)
    print(f"   Saved final feature order ({len(final_feature_order)} features) to {feature_order_path.name}", flush=True)
    ...
    joblib.dump(final_model, ARTIFACTS_DIR / "model_v5.pkl")
    joblib.dump(baseline_model, ARTIFACTS_DIR / "model_v5_baseline_logit.pkl")
    
    # Save CV fold indices
    with open(ARTIFACTS_DIR / "cv_fold_indices_v5.json", "w", encoding="utf-8") as f:
        json.dump(cv_indices, f, indent=2)
    
    # Save selected features
    with open(ARTIFACTS_DIR / "selected_features_v5.json", "w", encoding="utf-8") as f:
    ...
    importance_df.to_csv(ARTIFACTS_DIR / "feature_importance_v5.csv", index=False)
    ...
    with open(ARTIFACTS_DIR / "feature_selection_report_v5.md", "w", encoding="utf-8") as f:
        f.write("# Feature Selection Report V5\n\n")
    ...
    with open(ARTIFACTS_DIR / "model_training_report_v5.md", "w", encoding="utf-8") as f:
        f.write("# Model Training Report V5\n\n")
        f.write(f"**Generated:** {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n\n")
        f.write("## Summary\n\n")
    ...
    print()
    print("=" * 80)
    print("Unit 4 V5 Training Complete!")
    print("=" * 80)
    print()
    print("Artifacts created:")
    print(f"  - model_v5.pkl")
    print(f"  - model_v5_baseline_logit.pkl")
    print(f"  - cv_fold_indices_v5.json")
    print(f"  - feature_importance_v5.csv")
    print(f"  - feature_selection_report_v5.md")
    print(f"  - model_training_report_v5.md")
    print(f"  - selected_features_v5.json")
    print(f"  - feature_order_v5.json")
    print()
