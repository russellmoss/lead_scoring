...
SELECT 
    *,
    ...
    -- TENURE BINS (handle continuous variable)
    CASE 
        WHEN current_firm_tenure_months < 12 THEN 'New'
        WHEN current_firm_tenure_months < 36 THEN 'Established'
        WHEN current_firm_tenure_months < 60 THEN 'Veteran'
        ELSE 'Long-term'
    END AS tenure_category,
    
    -- FIRM SIZE PROXY (if we have rep count or can derive it)
    CASE 
        WHEN firm_departures_12mo + firm_arrivals_12mo > 50 THEN 'Large'
        WHEN firm_departures_12mo + firm_arrivals_12mo > 20 THEN 'Medium'
        ELSE 'Small'
...
df_verify = client.query(verify_sql).result().to_dataframe()
...
for _, row in df_verify.iterrows():
    if row['split_set'] == 'TRAIN':
        ...
        if row['conv_highly_mobile'] and row['conv_stable']:
